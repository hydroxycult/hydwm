 ...
 * events about window (dis-)appearance.
 * Only one X connection at a time is
 * allowed to select for this event mask.
 * *
 * The event handlers of dwm are organized in an array which is accessed
 * whenever a new event has been fetched.
 * This allows event dispatching
 * in O(1) time.
 *
 ...
 * on each monitor.
 * Each client contains a bit array to indicate the tags of a
 * client.
 * *
...
void buttonpress(XEvent *e) {
  ...
  if (ev->window == selmon->barwin) {
    i = x = 0;
    do {
      /* Use ball icons for width calculation */
      const char *tagtext;
      if (selmon->tagset[selmon->seltags] & 1 << i) {
        tagtext = tagicon_selected;
      } else if (selmon->clients && (selmon->clients->tags & 1 << i)) {
        tagtext = tagicon_occupied;
      } else {
        tagtext = tagicon_empty;
      }
      x += TEXTW(tagtext);
...
void drawbar(Monitor *m) {
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    const char *tagtext;
    /* Determine which icon to use based on tag state */
...
void manage(Window w, XWindowAttributes *wa) {
  ...
  updatesizehints(c);
  updatewmhints(c);
  // always center patch, also made it take into account the height of the bar -
  // mh
...
void tag(const Arg *arg) {
  if (selmon->sel && arg->ui & TAGMASK) {
    ...
    focus(NULL);
    arrange(selmon);
    view(arg); // Automatically switch to the workspace we just moved the window
               // to
  }
}
...
void togglebar(const Arg *arg) {
  selmon->showbar = selmon->pertag->showbars[selmon->pertag->curtag] =
      !selmon->showbar;
  updatebarpos(selmon);
  XMoveResizeWindow(dpy, selmon->barwin, selmon->wx + (floatbar ? gappov : 0),
                    selmon->by, selmon->ww - (floatbar ? 2 * gappov : 0), bh);
  arrange(selmon);
}
...
void updatebars(void) {
  ...
  for (m = mons; m; m = m->next) {
    ...
    m->barwin = XCreateWindow(
        dpy, root, m->wx + (floatbar ? gappov : 0), m->by,
        m->ww - (floatbar ? 2 * gappov : 0), bh, 0, DefaultDepth(dpy, screen),
...
void updatebarpos(Monitor *m) {
  ...
  if (m->showbar) {
    if (floatbar) {
      m->wh = m->wh - gappoh - bh;
      m->by = m->topbar ? m->wy + gappoh : m->wy + m->wh;
    } else {
      m->wh -= bh;
