...
static const char *blueberry[] = { "blueberry", NULL }; 
...
static const Key keys[] = {
	...
	// ===== LAUNCHERS & APPS ===== (ALT+Space launcher preserved as requested)
	{ ALTKEY,                       XK_space,              spawn,          {.v = dmenucmd } },                                // app launcher (rofi-like)
	{ ControlMask|ALTKEY,           XK_t,                  spawn,          {.v = termcmd  } },                               // terminal CTRL+ALT+T (Hyprland)
	{ ControlMask,                  XK_e,                  spawn,          {.v = thunar } },                                  // file manager CTRL+E (Hyprland)
	{ ControlMask|ShiftMask,        XK_z,                  spawn,          {.v = zenbrowser } },                              // zen-browser CTRL+SHIFT+Z (Hyprland)
	{ MODKEY,                       XK_m,                  spawn,          {.v = vscodium } },                                // vscodium SUPER+M (Hyprland)
	{ MODKEY,                       XK_l,                  spawn,          SHCMD("slock") },                                  // lock screen SUPER+L (Hyprland)
	{ MODKEY|ShiftMask,             XK_r,                  spawn,          SHCMD("$TOGGLE_ROFI $ROFI_MENUS/run") },           // rofi run menu
	{ MODKEY|ShiftMask,             XK_v,                  spawn,          SHCMD("$TOGGLE_ROFI $ROFI_MENUS/clipboard") },     // rofi clipboard
	{ MODKEY|ShiftMask,             XK_e,                  spawn,          SHCMD("$TOGGLE_ROFI $ROFI_MENUS/filebrowser") },   // rofi file browser
	{ MODKEY,                       XK_Escape,             spawn,          SHCMD("$TOGGLE_ROFI $ROFI_MENUS/logout") },        // rofi logout menu
	{ MODKEY,                       XK_t,                  spawn,          SHCMD("swaync-client -t || dunstctl history-pop") }, // notification center
        { ControlMask,                  XK_space,              spawn,          SHCMD("xcolor | xclip -selection clipboard") }, // colorpicker	
	// ===== WINDOW MANAGEMENT =====
	{ ControlMask,                  XK_q,                  killclient,     {0} },                                             // kill window CTRL+Q (Hyprland)
	{ ControlMask|ShiftMask,        XK_q,                  spawn,          SHCMD("xkill") },                                  // force kill CTRL+SHIFT+Q
	{ MODKEY,                       XK_v,                  togglefloating, {0} },                                             // toggle float SUPER+V (Hyprland)
	{ MODKEY,                       XK_f,                  togglefullscr,  {0} },                                             // fullscreen SUPER+F (Hyprland)
	{ MODKEY|ShiftMask,             XK_z,                  unfloatvisible, {.v = &layouts[0]} },                              // unfloat windows
	{ MODKEY,                       XK_u,                  spawn,          SHCMD("echo 'pin not supported in dwm'") },        // pin (dwm doesn't support)
	{ MODKEY,                       XK_w,                  spawn,          SHCMD("echo 'center window - use SUPER+V to float then manually center'") },
	{ MODKEY,                       XK_g,                  setlayout,      {.v = &layouts[13]} },                             // set workspace to float
	{ MODKEY,                       XK_Tab,                focusstack,     {.i = +1 } },                                      // cycle next (like ALT+TAB)
	{ ALTKEY,                       XK_Tab,                focusstack,     {.i = +1 } },                                      // ALT+TAB cycle (Hyprland)
	
	// ===== ARROW KEY NAVIGATION ===== (Hyprland-style)
	{ MODKEY,                       XK_Left,               focusstack,     {.i = -1 } },                                      // focus left
	...
	// ===== VIM-STYLE NAVIGATION ===== (keeping for compatibility)
	...
	{ ControlMask|ALTKEY,           XK_Left,               view,           {.ui = ~0 } },                                     // prev workspace (show all then cycle)
	...
	{ MODKEY,                       XK_z,                  spawn,          SHCMD("echo 'resize -25% not available'") },       // resize window (Hyprland)
	{ MODKEY,                       XK_c,                  spawn,          SHCMD("echo 'resize +25% not available'") },       // resize window (Hyprland)
	
	// ===== VOLUME & BRIGHTNESS ===== (Hyprland scripts - update paths as needed)
	{ MODKEY,                       XK_minus,              spawn,          SHCMD("$SCRIPTS/change_volume -2 || pactl set-sink-volume @DEFAULT_SINK@ -2%") },
	{ MODKEY,                       XK_equal,              spawn,          SHCMD("$SCRIPTS/change_volume +2 || pactl set-sink-volume @DEFAULT_SINK@ +2%") },
	{ 0,                            XF86XK_AudioLowerVolume, spawn,        SHCMD("$SCRIPTS/change_volume -5 || pactl set-sink-volume @DEFAULT_SINK@ -5%") },
	{ 0,                            XF86XK_AudioRaiseVolume, spawn,        SHCMD("$SCRIPTS/change_volume +5 || pactl set-sink-volume @DEFAULT_SINK@ +5%") },
	{ 0,                            XF86XK_AudioMute,      spawn,          SHCMD("pactl set-sink-mute @DEFAULT_SINK@ toggle") },
	{ 0,                            XF86XK_AudioMicMute,   spawn,          SHCMD("$SCRIPTS/toggle_microphone || pactl set-source-mute @DEFAULT_SOURCE@ toggle") },
	{ 0,                            XF86XK_MonBrightnessUp, spawn,         SHCMD("$SCRIPTS/brightness 4%+ || brightnessctl set +4%") },
	{ 0,                            XF86XK_MonBrightnessDown, spawn,       SHCMD("$SCRIPTS/brightness 4%- || brightnessctl set 4%-") },
	
	// ===== MEDIA CONTROLS ===== (Hyprland-style)
	...
	// ===== SCREENSHOTS ===== (Hyprland → X11 alternatives)
	{ 0,                            XK_Print,              spawn,          SHCMD("$SCRIPTS/screenshot || flameshot gui") },   // screenshot
	{ MODKEY|ShiftMask,             XK_s,                  spawn,          SHCMD("/home/luma/.local/bin/screenshot") },                          // area screenshot
	{ ControlMask,                  XK_Print,              spawn,          SHCMD("$SCRIPTS/toggle_recording || simplescreenrecorder") },
	{ ControlMask|ShiftMask,        XK_Print,              spawn,          SHCMD("$SCRIPTS/toggle_recording --select || simplescreenrecorder") },
	
	// ===== MONITOR MANAGEMENT =====
	{ MODKEY|ShiftMask,             XK_comma,              tagmon,         {.i = -1 } },                                      // move to prev monitor
	{ MODKEY|ShiftMask,             XK_period,             tagmon,         {.i = +1 } },                                      // move to next monitor
	
	// ===== TAG KEYS ===== (ALT+[1-6] for Hyprland compatibility, SUPER for advanced)
	TAGKEYS(                        XK_1,                                  0)
	TAGKEYS(                        XK_2,                                  1)
	TAGKEYS(                        XK_3,                                  2)
	TAGKEYS(                        XK_4,                                  3)
	...
	// ===== SYSTEM =====
	{ MODKEY|ControlMask,           XK_r,                  spawn,          SHCMD("pkill dwm") },                              // reload dwm (Hyprland hyprctl reload)
...
static const Button buttons[] = {
	...
	{ ClkStatusText,        0,              Button3,        spawn,          {.v = blueberry } },
